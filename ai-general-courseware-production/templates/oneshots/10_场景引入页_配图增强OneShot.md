...
固定 Demo SHA-256：`ce8cec951513ad6cd065106bcd65e8f80701972deef3e66319321c1b90bbf347`  
变量区外 SHA-256：`7ba0e176637d0065758f6ac94450d6a4951b53864c0769295b720827828e989e`
...
2. 配图变量锚点为精确行 `    const VISUAL_DATA = Object.freeze({});`，内容变量开始锚点为精确行 `    const SCENE_INTRO_VARIABLES = Object.freeze({`。
3. 内容变量区结束锚点为精确行 `    /* ======================= 变量区结束 ======================= */`；仅这两个变量值可由 S6 替换。
4. 从开始锚点首字节到结束锚点所在行的换行符为止，整段从哈希输入中移除；模型输出也先按第 1 步规范末尾 LF。
5. 拼接前后剩余字节并计算 SHA-256，必须得到 `7ba0e176637d0065758f6ac94450d6a4951b53864c0769295b720827828e989e`。
...
配图增强合同：必须从 `VISUAL_DATA.visualAsset` 或 `VISUAL_DATA.planVisualAssets` 逐项原序渲染，不得增删、改写 URL、alt、displayLabel 或 placement。图片是正文主配图，禁止缩略图、小装饰条或图标尺寸；必须按 `VISUAL_DATA.visualPresentation` 使用内容区宽度、自然比例、`object-fit: contain`、`16px` 圆角和 `16px` 上下间距，不额外套独立卡片。场景引入按 `scene_context_image` 融入剧本纸，放在情境段落之后、“镜头拉近”之前。图片本体作为唯一放大触发器，不显示“查看大图”按钮；同页 `.visual-lightbox` 必须使用 `.visual-lightbox-dialog`、`.visual-lightbox-stage` 和圆形 × 的 `.visual-lightbox-close`，图片水平、纵向居中，关闭控件水平居中放在图片底部与视口底部之间的纵向中点，遮罩内不得重复图片名或说明。必须支持点击遮罩、`Escape` 关闭，以及 `touchstart`/`touchmove` 两指 1–4 倍缩放和放大后的单指平移；关闭时调用 `resetVisualTransform` 复位，关闭按钮不得随图片变换。禁止外链预览、打开新窗口或新标签页；放大交互不得调用 CreatorReviewSDK。
...
- 不得重排、改写、删减或补写学生可见内容。
- 不得修改 VISUAL_DATA 与 SCENE_INTRO_VARIABLES 之外的任何 HTML、CSS、JavaScript、图片地址、固定文案、模块顺序、按钮或 SDK 行为。
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .scene-header h1 { font-size: var(--runs-type-h1-size); line-height: var(--runs-type-h1-line); font-weight: var(--runs-type-h1-weight); }
    .scene-content p, .director-cue p { font-size: var(--runs-type-body-size); line-height: var(--runs-type-body-line); font-weight: var(--runs-type-body-weight); }
...
<body>
  ...
  <script>
    /* ============================================================
       每课只允许替换 VISUAL_DATA 与本内容变量区中的原文内容及 pageAction。
